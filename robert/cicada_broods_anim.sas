...
/* determine whether or not each brood is active in the given year */
data temp_data; set cicada_data;
test_year=&year;
year_diff=(example_year-test_year);
if (example_year-test_year)/cycle = int((example_year-test_year)/cycle) then test='y';
run;

/* subset it to just the active ones */
data temp_data; set temp_data (where=(test='y'));
run;
...
x=87; position='6';
y=y_anchor;    text='Brood I'; output;
y=y-linespace; text='Brood II'; output;
y=y-linespace; text='Brood III'; output;
y=y-linespace; text='Brood IV'; output;
y=y-linespace; text='Brood V'; output;
y=y-linespace; text='Brood VI'; output;
y=y-linespace; text='Brood VII'; output;
y=y-linespace; text='Brood VIII'; output;
y=y-linespace; text='Brood IX'; output;
y=y-linespace; text='Brood X'; output;
y=y-linespace; text='Brood XIII'; output;
y=y-linespace; text='Brood XIV'; output;
y=y-linespace;
y=y-(linespace/2);
y=y-linespace; text='Brood XIX'; output;
y=y-linespace; text='Brood XXII'; output;
y=y-linespace; text='Brood XXIII'; output;
y=y-(linespace/2);
...
/* Get the annotated legend obsns for the active broods, so you can 'hilight' them */
proc sql noprint;
create table anno_hilight as
select anno_legend.*
from anno_legend 
where text in (select unique brood_roman from active_broods);
quit; run;
data anno_hilight; set anno_hilight;
...
title;
footnote;
/* title1 ls=1.5 "Cicada Broods in &year"; */
data anno_title;
length function $8 style $30 text $100;
xsys='3'; ysys='3'; hsys='3'; when='a';
function='label'; 
...
text="Cicada Broods in &year"; output;
...
length function $8 color $20 style $30;
 ...
 anno_county_outlines 
 anno_state_outlines 
 anno_legend anno_legend_outlines anno_hilight;
run;
...
options dev=sasprtc printerpath=gif animduration=1.4 animloop=0 
...
data loopdata; 
/*
 do year=2020 to 2030 by 1;
 do year=2020 to 2022 by 1;
 do year=2020 to 2037 by 1;
 do year=2020 to 2054 by 1;
*/
 do year=2020 to 2054 by 1;
 output;
 end;
...
data _null_; set loopdata;
 call execute('%do_year('|| year ||');');
run;

/* output a blank slide, before the animation loops back to the beginning */
title;
footnote;
proc gslide
 des='' name="&name";
